...
export default function CantileverBeamConcentratedLoad() {
  const [length, setLength] = useState(1); // Giá trị mặc định là 1
  const [loadValue, setLoadValue] = useState(10); // Giá trị mặc định là 10, bây giờ là giá trị P
  ...
  function calculateDeflection(x, length, loadValue, elasticModulus, momentOfInertia) {
    ...
    return  (P * (2*Math.pow(L, 3)-3 * Math.pow(L, 2) * x + Math.pow(x, 3)) / (6 * E * I))  * Math.pow(10, 12) // Đổi đơn vị từ m sang mm
  }
  ...
  const {
    ...
    // Thêm elasticModulus vào dependency array
  ...
  const handleSubmit = (event) => {
    ...
    const newMaxDeflection =
      (loadValue * Math.pow(length, 3)) /
      (3 * elasticModulus * newMomentOfInertia * Math.pow(10,-12));

    // Cập nhật state
    setShearForceData(calculatedShearForceData);
    setBendingMomentData(calculatedBendingMomentData);
    setMaxBendingMoment(
      parseFloat(calculateBendingMoment(length, loadValue, length).toFixed(2)) // Mô men uốn Mmax = P * L (tại ngàm)
    );
    setMaxShearForce(
      parseFloat(calculateShearForce(length, loadValue, length).toFixed(2)) // Lực cắt Vmax = P
    );
  ...
  return (
    <main>
      {/* Thay đổi tiêu đề */}
      <h1>Dầm ngàm một đầu, chịu tải trọng tập trung tại đầu tự do</h1>

      <div className="input-area">
        <div>
          {/* Thay đổi hình ảnh minh họa */}
          <Image
            src="/images/cantilever-beam-concentrated-load.png"
            alt="Sơ đồ dầm ngàm một đầu chịu tải trọng tập trung tại đầu tự do"
            width={500}
            height={300}
