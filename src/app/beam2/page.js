...
export default function CantileverBeam() {
  ...
  function calculateShearForce(length, loadValue, x) {
    const q = loadValue;
    return q * x; // Lực cắt Vx = qx
  }

  // Hàm tính moment uốn
  function calculateBendingMoment(length, loadValue, x) {
    const q = loadValue;
    return (-q * Math.pow(x, 2)) / 2; // Mômen uốn Mx = qx^2 / 2
  }
  ...
  function calculateDeflection(x, length, loadValue, elasticModulus, momentOfInertia) {
    ...
    return (
      Math.pow(10, 12)*(q * (Math.pow(x, 4) - 4 * Math.pow(L, 3) * x + 3 * Math.pow(L, 4))) /
      (24 * E * I) 
    ); // Đơn vị mm
  }
  ...
  const handleSubmit = (event) => {
    ...
    const newMaxDeflection =
      (loadValue * Math.pow(length, 4)) /
      (8 * elasticModulus * newMomentOfInertia * Math.pow(10, -12));

    // Cập nhật state
    setShearForceData(calculatedShearForceData);
    setBendingMomentData(calculatedBendingMomentData);
    setMaxBendingMoment(
      parseFloat(calculateBendingMoment(length, loadValue, length).toFixed(2))
    );
    setMaxShearForce(
      parseFloat(calculateShearForce(length, loadValue, length).toFixed(2))
    );
  ...
  return (
    <main>
      <h1>Dầm ngàm, tải phân bố đều</h1>

      <div className="input-area">
        <div>
          {/* Thay đổi hình ảnh minh họa */}
          <Image
            src="/images/cantilever-beam-diagram.png"
            alt="Sơ đồ dầm ngàm chịu tải phân bố đều"
            width={500}
            height={300}
